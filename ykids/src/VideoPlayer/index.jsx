...
const VideoPlayer = ({
  ...
  const [showTask, setShowTask] = useState(settings.taskMode);
  const activeVideoIndex = defaultVideoIndex;
  const hoverVideoIndex = defaultHoverIndex;
  useEffect(() => {
    setPlaying(defaultVideoIndex >= 0);
  }, [defaultHoverIndex]);
  ...
  return (
    <div className={"main " + (isUserActive ? "" : "userUnactive")}>
      ...
      <div style={{ flexGrow: 1, position: "relative" }}>
        <ReactPlayer
          url={
            JOINED_DATA.length > 0
              ? JOINED_DATA[activeVideoIndex >= 0 ? activeVideoIndex : 0].url
              : DATA[0].url
          }
          playing={playing}
          controls={false}
        ...
        />
        <div className={"playerLayer " + (settings.onlySound ? "dark" : "")}>
          <div
            onClick={() => {
              setActiveVideoIndex(-1);
              if (settings.taskMode) {
